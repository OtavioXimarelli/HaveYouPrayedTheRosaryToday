...
export function PwaInstallPrompt() {
  ...
  return (
    <aside className="install-sheet" aria-label={t('installTitle')}>
      <p className="install-copy">{t('installTitle')}</p>
      <div className="install-actions">
        <button type="button" className="button button-small" onClick={install}>{t('installAction')}</button>
        <button type="button" className="text-link" onClick={close}>{t('installLater')}</button>
      </div>
    </aside>
  );
}
