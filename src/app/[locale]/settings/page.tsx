...
export default function SettingsPage() {
  ...
  const save = (event: FormEvent) => { event.preventDefault(); setMessage(t('saveSuccess')); };
  const exportData = () => {
  ...
  const clearData = () => {
    ...
    prayer.resetPrayerData(); preferences.resetPreferences(); setMessage(t('clearDone')); router.refresh();
  };
    ...
    <div className="page-wrap settings-page">
      ...
      <div className="dashboard-grid settings-grid">
        <section className="paper-panel settings-main"><h2 className="section-title" style={{fontSize: '2rem'}}>{t('profileTitle')}</h2><form className="form-stack" onSubmit={save} style={{marginTop: '2rem'}}>
          <div className="field"><label htmlFor="settings-name">{t('nameLabel')}</label><input id="settings-name" value={preferences.firstName} onChange={(event) => preferences.setProfile({firstName: event.target.value})} /></div>
        ...
        <aside className="paper-panel settings-aside"><span className="settings-aside-mark" aria-hidden="true">✣</span><h2 style={{fontFamily: 'var(--font-serif)', fontSize: '1.5rem'}}>{t('dataTitle')}</h2><div className="form-stack"><button className="button button-secondary" onClick={exportData}>{t('export')}</button><button className="button button-secondary" onClick={clearData}>{t('clear')}</button><Link href="/privacy" className="text-link">{t('privacyLink')}</Link><Link href="/inicio" className="text-link">{t('publicHome')}</Link></div></aside>
      </div>
    </div>
  );
}
